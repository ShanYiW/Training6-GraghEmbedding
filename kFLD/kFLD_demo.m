...
%     Ke = (trainset'*testset).^p; % Ntr*Nte
    K = exp(-dist(cv(is_train),cv(is_train))./sig2); % 高斯核
    Ke = exp(-dist(cv(is_train), cv(is_test))./sig2); % 
    %% kPCA
%     [Alpha] = kFLD_poly(trainset, gndtrain, p);
    [Alpha] = kFLD_gs(trainset, gndtrain, sig2);
    for d=lb_d:ub_d
%         Ytrain = Alpha(:,1:d)'*(K - K*NN1 - NN1*K + NN1*K*NN1); % 中心化
%         Ytest = Alpha(:,1:d)'*(Ke - K*NNe1 - NN1*Ke + NN1*K*NNe1);
        Ytrain = Alpha(:,1:d)'*K; % 未中心化
        Ytest = Alpha(:,1:d)'*Ke; % 
        acc = 0;
        for j=1:Ntest
            y = Ytest(:,j); % d*1
