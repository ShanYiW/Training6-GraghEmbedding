...
lb_d=2; ub_d=Ntrain-N_class; len_d=ub_d-lb_d+1;
...
for rnd = 1:N_rnd
    %% 生成训练集, 测试集
    train_idx = sort(randperm(N_per_class,L)); % 1*L  升序排序
    ...
    %% 运行 PCA, 输出E
    mTrain = mean(trainset, 2); % D*1
    trainset = trainset - repmat(mTrain, [1,Ntrain]);
    [E, Eval] = eig(trainset*trainset');
    Eval = diag(Eval); % D*1
    [~, idx_e] = sort(-Eval); % 降序排序
    E = E(:, idx_e); % D*D
    %% 计算分类准确度
    trainset = trainset + mTrain;
    for d=lb_d:ub_d
